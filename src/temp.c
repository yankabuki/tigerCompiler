...
Temp_tempList Temp_copy(Temp_tempList a) {
	if(a == NULL)
		...
		return Temp_TempList(a->head, Temp_copy(a->tail));
}

Temp_tempList Temp_union(Temp_tempList a, Temp_tempList b) {
	Temp_tempList ret = Temp_copy(a), i;
	for(i = b; i; i = i->tail)
		if(!Temp_in(ret, i->head))
			ret = Temp_TempList(i->head, ret);
	return ret;
}
...
void Temp_printList(Temp_tempList list) {
	if(list == NULL)
		...
		printf("%s", Temp_look(Temp_name(), list->head));
		if(list->tail)
			printf(", ");
		Temp_printList(list->tail);
	}
}
